...
section "Widget routes are session-gated"
for route in handoff activity messages leads reset booking; do
...
expect 401 "GET /widget/v1/sessions/{id}/messages needs a session token" "$API/widget/v1/sessions/x/messages"

section "Cross-origin rules"
...
if [ -z "$AGENT_KEY" ] || [ -z "$ORIGIN" ]; then
  ...
  section "A real conversation, end to end"
