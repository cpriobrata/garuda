...
export function SignUpForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      clearAuthSession();
      const result = await garudaApi.signUp(`${form.get("firstName")} ${form.get("lastName")}`.trim(), String(form.get("email")), String(form.get("password")));
  ...
  if (pendingEmail) {
    return (
      <div className="space-y-5">
        ...
        <Button type="button" variant="outline" size="lg" className="w-full" disabled={submitting} onClick={resend}>{submitting ? "Sending..." : "Resend verification email"}</Button>
        <Button type="button" variant="ghost" className="w-full" onClick={() => { setPendingEmail(""); setResendMessage(""); }}><ArrowLeft className="mr-2 h-4 w-4" /> Use a different email</Button>
      </div>
    );
  }
  ...
  return (
    <form className="space-y-5" onSubmit={submit}>
      ...
      {error && <p role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{error}</p>}
      <Button type="submit" size="lg" className="w-full" disabled={submitting}>{submitting ? "Creating account…" : "Continue to plan"} <ArrowRight className="ml-2 h-4 w-4" /></Button>
