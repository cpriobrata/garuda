...
export function SignInForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      clearAuthSession();
      const result = await garudaApi.signIn(email, String(data.get("password")));
  ...
  async function resendVerification() {
    if (!resendEmail || resendBusy || resendCooldown > 0) return;
    setResendBusy(true);
    setResendMessage("");
    try {
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      {resendEmail && <div className="space-y-2 rounded-xl border border-indigo-200 bg-indigo-50 p-3"><p className="text-xs leading-5 text-indigo-700">Verification email: <span className="font-semibold">{resendEmail}</span></p><Button type="button" variant="outline" className="w-full border-indigo-200 bg-white" disabled={resendBusy || resendCooldown > 0} onClick={resendVerification}>{resendBusy ? "Sending..." : resendCooldown > 0 ? `Resend available in ${resendCooldown}s` : "Resend verification email"}</Button>{resendMessage && <p role="status" className="text-[10px] leading-4 text-indigo-700">{resendMessage}</p>}</div>}
      <Button type="submit" size="lg" className="w-full">Sign in to Garuda <ArrowRight className="ml-2 h-4 w-4" /></Button>
