...
export function SettingsPanel() {
  ...
  useEffect(() => {
    garudaApi.me().then((value) => setProfile({ name: value.user.name || value.user.email.split("@")[0], email: value.user.email, organization: value.organization.name })).catch(() => undefined);
  }, []);
...
function SettingsCard({ title, description, action, children }: { title: string; description: string; action?: React.ReactNode; children: React.ReactNode }) {
  return <section className="rounded-xl border bg-white shadow-sm"><div className="flex items-start justify-between border-b p-5 sm:p-6"><div><h2 className="text-sm font-semibold text-slate-900">{title}</h2><p className="mt-1 text-xs text-slate-500">{description}</p></div>{action}</div><div className="p-5 sm:p-6">{children}</div></section>;
}
